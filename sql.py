...
class alumni(UserMixin,db.Model):
    ...
    approve=db.Column(db.Integer(),default=0)
    date_of_register=db.Column(db.String(100))
...
@app.route('/alumnis')
def alumnis(): 
    id=db.engine.execute(f"SELECT * from alumni where approve=1")
    return render_template('alumnis.html',data=id)
...
def addevent():
    ...
    if request.method=="POST":
        ...
        atte=event(EventName=eventname,date_of_conduct=date,eventc=eventc,headline=headline,register=register)
        db.session.add(atte)
        db.session.commit()
...
def addalumni():
    if request.method=="POST":
        ...
        query=db.engine.execute(f"INSERT INTO `alumni` (`name`,`designation`,`linkdin`,`company`,`email`,`about`,`date_of_register`) VALUES ('{name}','{desi}','{linkdin}','{company}','{email}','{about}' ,'{date}')")
        flash("registered Confirmed","info")
